...
export class SceneVisualizationComponentService {
  ...
  public getPointedElement(event: MouseEvent, scene: ThreeJS.Scene): null | ThreeJS.Intersection {
    ...
    const x :number= (event.offsetX / (event.target as HTMLCanvasElement).offsetWidth) * 2 - 1;
    const y:number = - (event.clientY / window.innerHeight) * 2 + 1;
    let passValue:any={ x: x, y: y };
    this._ray.setFromCamera(passValue, this._camera!);
  ...
  private _render(scene: Scene) {
    // console.log( this._renderer,"scene")
