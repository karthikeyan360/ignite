...
export class VisualizerComponentService implements IVisualizerContextService {
  ...
  public async hoverGoodById(goodId: string | null) {
    ...
    const goods :any= [];
    // await selectSnapshot(this._store.select(selectCurrentSolutionGoods));
    this._hoveredGood.next(goods.find((good:any) => good.id === goodId) ?? null);
  }
  ...
  public async selectGoodById(goodId: string | null) {
    ...
    const goods :any=[];
    // await selectSnapshot(this._store.select(selectCurrentSolutionGoods));
    this._selectedGood.next(goods.find((good:any) => good.id === goodId) ?? null);
  }
}
